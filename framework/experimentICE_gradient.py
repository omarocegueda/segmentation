...
results_dir = '/home/dalmau/opt/segmentation/framework/experimentICE_gradient_output/example2_30wCoronal/se_diam7/'
atlas_label = '28w'
se_diam = 7  # {3,5,7}   <-- structuring element diameter for morphological gradient
...
cross_se = np.array(np.zeros((se_diam,se_diam,se_diam)),dtype=np.uint8) # <-- should have 3 voxel units of diameter
i_mid = int(se_diam/2.0)
for i in xrange(0,se_diam):
